...
class MediaLibraryIntegration {
    ...
    public function injectBulkSelectionBridge(): void {
        // Selection bridge + sidebar/portal positioning, delivered as a real
        // enqueued file (no inline buffer / no raw <script> tag). Enqueued from
        // enqueueAssets() where the bundle handle is already registered.
        wp_register_script(
            'mediapilot-media-bridge',
            MDPAI_URL . 'admin/assets/js/mediapilot-media-bridge.js',
            [ 'mediapilot-admin' ],
            MDPAI_VERSION,
            true
        );
        wp_localize_script(
            'mediapilot-media-bridge',
            'MediaPilotMediaBridge',
            [
                'i18n' => [
                    'unusedTitle' => __( 'Not used anywhere — safe to review before deleting', 'mediapilot-ai' ),
                ],
            ]
        );
        wp_enqueue_script( 'mediapilot-media-bridge' );
    }
